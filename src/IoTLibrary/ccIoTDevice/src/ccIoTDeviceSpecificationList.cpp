...
ccIoTDeviceSpecificationList& ccIoTDeviceSpecificationList::operator=(ccIoTDeviceSpecificationList& other) {
    ...
    this->specification_json_ = other.specification_json_;
    this->specification_list_ = other.specification_list_;

    return *this;
}

bool ccIoTDeviceSpecificationList::load_file(const std::string& file_name) {
...
bool ccIoTDeviceSpecificationList::parse(const Json::Value& info) {
    ...
    if (info.isArray() == false) {
        std::shared_ptr<ccIoTDeviceSpecification> device_spec = std::make_shared<ccIoTDeviceSpecification>();

        if (device_spec->parse(info))
            specification_list_.push_back(device_spec);
    } else {
...
std::vector<std::shared_ptr<ccIoTDeviceSpecification>>&  ccIoTDeviceSpecificationList::get_list() {
...
bool ccIoTDeviceSpecificationList::has_device(const std::string& device_name) {
    return has_device(ccIoTDeviceSpecification::get_type(device_name));
}
